...
- [ ] **Avoid magic strings for buckets and models**
  - **Problem**: Bucket names (`"documents"`, `"voice_messages"`) and model names (`"gemini-2.5-flash"`, `"gemini-embedding-001"`) are duplicated across routes, config, and edge functions.
  - **Proposed solution**: Export constants from a single config module (e.g. `DOCUMENTS_BUCKET`, `VOICE_BUCKET`, `DEFAULT_CHAT_MODEL`, `DEFAULT_EMBEDDING_MODEL`) and import them wherever needed (chat provider, RAG, ingest routes, docs delete route, audio upload route).
...
- [ ] **Expose retrieval configuration more flexibly**
  - **Problem**: Retrieval currently only exposes `topK` from the chat API and uses a fixed `minSimilarity` threshold; the UI has no way to adjust retrieval strictness.
  - **Proposed solution**: Add an optional RAG config input (per request or per chat) that allows tuning `topK`, `minSimilarity`, and `maxHistoryPairs`. Thread this config from the UI → `/api/chat` → `searchRelevantChunks` / `buildPrompt`.

- [ ] **RAG prompt behavior tests**
  - **Problem**: Prompt building and citation grouping are central but untested; regressions in how context is constructed would be easy to miss.
  - **Proposed solution**: Add small unit tests for `buildPrompt` (history trimming, system prompt injection, doc grouping, SOURCE formatting) using a test runner like Vitest or Jest.

---

### 3. Streaming, SSE, and Chat UX
...
- [ ] **Handle all SSE event types on the client**
  ...
  - **Proposed solution**: Extend `parseSSEStream` to recognize `"limit"` and `"error"` events and surface them as structured events. When `"limit"` is received, set a `_limitNotice` on the active assistant message. When `"error"` is received, set `_error` on the message and a user-facing `globalError` for the banner.

- [ ] **Extract SSE parsing into a reusable helper**
  - **Problem**: SSE parsing logic is embedded inside `useChatApp`, which makes it harder to test and reuse.
  - **Proposed solution**: Move `parseSSEStream` into a small helper (e.g. `features/chat/lib/sse.ts`) with a typed event union. Add a couple of tests for partial chunks, multiple events in one chunk, and malformed lines.

---

### 4. Auth, API Ergonomics & Multi-tenancy
...
### 5. Ingestion Pipeline & Document Management

- [ ] **Centralize bucket names and share between Next & edge**
  - **Problem**: Bucket name `"documents"` appears in the ingest API route, docs delete route, and edge functions, which increases the risk of a mismatch.
  - **Proposed solution**: Define a single `DOCUMENTS_BUCKET` constant (or env) and use it in both the Next.js side (`/api/ingest`, `/api/docs/[id]`) and edge functions (via `_shared/ingest.ts`). Do the same for any other buckets (e.g. `VOICE_BUCKET`).

- [ ] **Surface ingest progress and errors in the UI**
  - **Problem**: The ingestion pipeline tracks detailed status and errors (`document_jobs`, `document_chunk_jobs`), but the UI only shows a coarse `status` on `documents`.
  - **Proposed solution**: Extend `/api/docs` to return a high-level progress summary (e.g. `processed_chunks / total_chunks`) and a simple error reason when a document is in `error`. Update `useDocsPage` and the document table to show progress and a short error label/tool-tip.

- [ ] **Add lightweight polling for document status**
  - **Problem**: The docs page requires a manual refresh to see status changes while ingestion is running.
  - **Proposed solution**: In `useDocsPage`, add periodic polling (e.g. every 3–5 seconds) while any document is in `pending` or `processing`, falling back to manual refresh-only when all docs are `ready` or `error`.

---

### 6. Frontend Structure & UX
...
### 7. Tooling, Testing & Documentation
...
- [ ] **Add initial test suite**
  ...
  - **Proposed solution**: Introduce a simple test setup (e.g. Vitest) and start with high-value units: `buildPrompt`, `chunkText`, and `parseDocument` (for text/markdown paths), plus a happy-path test for `/api/chat` using a mocked Gemini provider.
...
### 8. Small Code Cleanups

- [ ] **Remove or wire up unused flags and parameters**
  - **Problem**: Some flags/params are unused or misleading (e.g. `USE_REAL_BACKEND` constant that is always `true`, unused `req` arguments in some routes).
  - **Proposed solution**: Either remove unused flags or make them configurable (via env) if they’re intended for mocking. Rename unused parameters to `_req`/`_params` or remove them to satisfy ESLint.
